...
export default function Generate() {
  ...
  const initialInput = new URLSearchParams(search).get("input") || "";
  ...
  async function handleGenerate(val?: string) {
    ...
    try {
      ...
      if (isGitHubUrl(v)) {
        ...
        if (r.ok) {
          meta = await r.json(); setRepoMeta(meta);
  ...
  return (
    <div style={{ background: BG, fontFamily: MONO, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{
        ...
        <button onClick={() => setLocation('/')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer' }}>
...
function CopyBtn({ text, id, copied, onCopy, label = 'Copy' }: {
  ...
  return (
    <button onClick={() => onCopy(text, id)}
      style={{
        display: 'flex', alignItems: 'center', gap: 5,
        background: 'none', border: `1px solid ${done ? BORDER : BORDER}`,
...
function GitHubModal({ repoName, setRepoName, token, setToken, loading, result, error, onClose, onSubmit }: {
  ...
  loading: boolean; result: any; error: string | null;
