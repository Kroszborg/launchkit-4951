  ...
  return (
    <div className="min-h-screen" style={{ background: '#0a0a0a' }}>
      ...
      <section className="max-w-5xl mx-auto px-6 pt-24 pb-20">
        <div className="animate-fade-up stagger-1">
          <div className="inline-flex items-center gap-2 border border-[#2a2a2a] px-3 py-1 mb-8 text-[#6b6b6b] text-xs font-['DM_Mono']">
            <span className="w-1.5 h-1.5 rounded-full bg-[#e8e0d0] inline-block"></span>
        ...
        >
          ...
          <em className="italic" style={{ color: '#c8bfaf' }}>a better introduction.</em>
        </h1>
        ...
        <div className="animate-fade-up stagger-4">
          <HeroInput onSubmit={(val) => setLocation(`/generate?input=${encodeURIComponent(val)}`)} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
          {features.map((f, i) => (
            <div
              ...
              style={{ borderRight: i < 2 ? undefined : undefined, marginLeft: i > 0 ? '-1px' : 0 }}
            >
              ...
              <p className="font-['DM_Mono'] text-[#6b6b6b] text-sm leading-relaxed">
                {f.desc}
              </p>
            </div>
          ))}
      ...
      <section className="border-t border-[#2a2a2a] max-w-5xl mx-auto px-6 py-24 text-center">
        <h2 className="font-['Playfair_Display'] text-[#e8e0d0] font-semibold mb-6"
        ...
        <button
          ...
          className="border border-[#e8e0d0] text-[#e8e0d0] px-8 py-3 font-['DM_Mono'] text-sm hover:bg-[#e8e0d0] hover:text-[#0a0a0a] transition-colors duration-200"
        >
      ...
      <footer className="border-t border-[#2a2a2a] px-6 py-6">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <span className="font-['DM_Mono'] text-[#3a3a3a] text-xs">Built on Runable</span>
        </div>
      </footer>
    </div>
  );
...
function HeroInput({ onSubmit }: { onSubmit: (val: string) => void }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-0 max-w-2xl border border-[#2a2a2a]">
      <input
        ...
        className="flex-1 bg-[#111] text-[#e8e0d0] placeholder-[#3a3a3a] px-5 py-4 font-['DM_Mono'] text-sm outline-none focus:border-[#e8e0d0] border border-transparent"
